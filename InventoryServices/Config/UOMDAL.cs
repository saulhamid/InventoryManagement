...
namespace InventoryServices.InventoryManagement
{
  public  class UOMDAL
  {
      ...
      {
          var Supplier = _context.UOMs.Where(m => m.IsActive == true && m.IsArchive == false).OrderBy(m => m.Name).Select(m => new { m.Id, m.Name }).ToList();
          return Supplier;
      }
      ...
      public IEnumerable<UOM> Autocomplete(string term)
      {
          IEnumerable<UOM> uoms = from uom in _context.UOMs
                                  where uom.IsArchive == false && uom.IsActive == true 
                                  && (uom.Code.Contains(term) || uom.Name.Contains(term))
                                  orderby uom.Name select new UOM() { Id = uom.Id, Name = uom.Name };
          return uoms.ToList();
      }
      #endregion Method
  }

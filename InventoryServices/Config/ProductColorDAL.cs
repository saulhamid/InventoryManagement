  ...
  public  class ProductColorDAL
    {
        ...
        {
            var ProductColors = _context.ProductColors.Where(m => m.IsActive == true && m.IsArchive == false).OrderBy(m => m.Name).Select(m => new { m.Id, m.Name }).ToList();
        ...
        public IEnumerable<ProductColor> Autocomplete(string term)
        {
            IEnumerable<ProductColor> ProductColors = from ProductColor in _context.ProductColors
                                    ...
                                    && (ProductColor.Code.Contains(term) || ProductColor.Name.Contains(term))

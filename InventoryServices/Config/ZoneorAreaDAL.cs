   ...
   public class ZoneorAreaDAL
    {
        ...
        public dynamic Dropdown()
        {
            ...
            var ZoneOrAreas = _context.ZoneOrAreas.Where(m => m.IsActive == true && m.IsArchive == false).OrderBy(m => m.Name).Select(m => new { m.Id, m.Name }).ToList();
            return ZoneOrAreas;
        }
        public IEnumerable<ZoneOrArea> Autocomplete(string term)
        {
            IEnumerable<ZoneOrArea> ZoneOrAreas = from ZoneOrArea in _context.ZoneOrAreas
                                    ...
                                    && (ZoneOrArea.Code.Contains(term) || ZoneOrArea.Name.Contains(term))

...
namespace InventoryServices.InventoryManagement
{
    public class SupplierDAL
    {
        ...
        {
            var Supplier = _context.Suppliers.Where(m => m.IsActive == true && m.IsArchive == false).OrderBy(m => m.Name).Select(m => new { m.Id, m.Name }).ToList();
            return Supplier;
        }
        public dynamic Autocomplete(string term)
        {
            var Suppliers = from Supplier in _context.Suppliers
                                              where Supplier.IsArchive == false && Supplier.IsActive == true
                                              && (Supplier.Code.Contains(term) || Supplier.Name.Contains(term))

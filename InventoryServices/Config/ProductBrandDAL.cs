   ...
   public class ProductBrandDAL
    {
        ...
        {
            var ProductBrand = _context.ProductBrands.Where(m => m.IsActive == true && m.IsArchive == false).OrderBy(m => m.Name).Select(m => new { m.Id, m.Name }).ToList();
            return ProductBrand;
        }
        public IEnumerable<ProductBrand> Autocomplete(string term)
        {
            IEnumerable<ProductBrand> ProductBrands = from ProductBrand in _context.ProductBrands
                                    ...
                                    && (ProductBrand.Code.Contains(term) || ProductBrand.Name.Contains(term))

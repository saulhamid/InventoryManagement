  ...
  public class ProductTypeDAL
    {
        ...
        {
            var ProductType = _context.ProductTypes.Where(m => m.IsActive == true && m.IsArchive == false).OrderBy(m => m.Name).Select(m => new { m.Id, m.Name }).ToList();
            return ProductType;
        }
        public dynamic Autocomplete(string term)
        {
            var ProductTypes = from ProductType in _context.ProductTypes
                                    where ProductType.IsArchive == false && ProductType.IsActive == true
                                    && (ProductType.Code.Contains(term) || ProductType.Name.Contains(term))

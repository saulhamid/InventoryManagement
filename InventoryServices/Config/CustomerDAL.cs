   ...
   public class CustomerDAL
    {
        ...
        {
            var Customer = _context.Customers.Where(m => m.IsActive == true && m.IsArchive == false).Select(m => new { m.Id, m.Name }).ToList();
            return Customer;
        }
        public dynamic Autocomplete(string term)
        {
            var Customers = from Customer in _context.Customers
                                    ...
                                    && (Customer.Code.Contains(term) || Customer.Name.Contains(term))
                                    orderby Customer.Name
                                    select new Customer() { Id = Customer.Id, Name = Customer.Code +'~'+Customer.Name };
